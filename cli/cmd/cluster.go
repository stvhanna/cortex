...
func clusterInit() {
	...
	_upCmd.Flags().BoolVarP(&_flagClusterYes, "yes", "y", false, "skip prompts")
	...
	_infoCmd.Flags().BoolVarP(&_flagClusterYes, "yes", "y", false, "skip prompts")
	...
	_updateCmd.Flags().BoolVarP(&_flagClusterYes, "yes", "y", false, "skip prompts")
	_clusterCmd.AddCommand(_updateCmd)

	_downCmd.Flags().SortFlags = false
	addClusterConfigFlag(_downCmd)
	_downCmd.Flags().BoolVarP(&_flagClusterYes, "yes", "y", false, "skip prompts")
...
var _upCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		awsCreds, err := getAWSCredentials(_flagClusterConfig, _flagClusterEnv, _flagClusterYes)
		if err != nil {
			exit.Error(err)
		}

		clusterConfig, err := getInstallClusterConfig(awsCreds, _flagClusterEnv, _flagClusterYes)
		if err != nil {
			exit.Error(err)
		}
...
var _updateCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		accessConfig, err := getClusterAccessConfig(_flagClusterYes)
		if err != nil {
			exit.Error(err)
		}
		...
		cachedClusterConfig := refreshCachedClusterConfig(awsCreds, _flagClusterYes)

		clusterConfig, err := getClusterUpdateConfig(cachedClusterConfig, awsCreds, _flagClusterYes)
		if err != nil {
			exit.Error(err)
		}
...
var _infoCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if _flagClusterInfoDebug {
			...
			cmdInfo(awsCreds, accessConfig, _flagClusterYes)
		}
	},
}
